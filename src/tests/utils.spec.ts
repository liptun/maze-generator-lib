...
import { MazeDimensions, MazeCell, MazeCellType, Direction } from '../types';
...
describe('queryForEmptySpace', () => {
  it('should return four possible directions', () => {
    let maze = createEmptyMaze(mazeDimensions);
    maze = updateMazeCell(maze, { x: 2, y: 3, type: MazeCellType.Empty });
    maze = updateMazeCell(maze, { x: 4, y: 3, type: MazeCellType.Empty });
    maze = updateMazeCell(maze, { x: 3, y: 2, type: MazeCellType.Empty });
    maze = updateMazeCell(maze, { x: 3, y: 4, type: MazeCellType.Empty });
